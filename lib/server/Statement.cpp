...
Statement::Statement(std::string handle, std::string sqlStatement, StatementType type,
                     std::unique_ptr<util::SharedSemaphore> sharedSemaphore)
   : handle(handle), sqlStatement(sqlStatement), type(type), sharedSemaphore(std::move(sharedSemaphore)) {}
