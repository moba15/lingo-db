...
struct TargetInfo {
   ...
   std::shared_ptr<ColumnReference> getTargetColumn(size_t i) {
      assert(!targetColumns[i].expired());
      return targetColumns[i].lock();
   }
   std::weak_ptr<ColumnReference>& operator[](size_t i ) {
   ...
   void setTargetColumns(std::vector<std::shared_ptr<ColumnReference>> colums) {
      targetColumns = std::vector<std::weak_ptr<ColumnReference>>(colums.begin(), colums.end());
   }
   ...
   private:
   std::vector<std::weak_ptr<ColumnReference>> targetColumns;
};
} // namespace lingodb::ast
#endif
